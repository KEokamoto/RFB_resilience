...
# REVISED: Fig 3 sample-level bars, Fig 5 two-panel, Fig 7 single global baseline,
#          Fig 6 uses concordance-filtered labels_row from 02_analysis.R

dir.create("figures", showWarnings = FALSE)

# -------------------------
# Figure 2 — G0 baseline alpha diversity
# Note: Fig 2 has been removed from the main paper per Reviewer 3.
# Statistics are now reported in text with reference to Fig 4.
# File retained here for reference/supplementary use if needed.
# -------------------------
...
ggsave("figures/Fig2_G0_baseline_alpha_diversity.png",
       plot = p_fig2, width = 6.85, height = 3.8, dpi = 300, bg = "white")

# -------------------------
# Figure 3 — shared flour/gut colour palette setup
# (required by both Fig 3a and 3b)
# -------------------------
...
# -------------------------
# Figure 3a — flour substrates: SAMPLE-LEVEL bars (Reviewer 3)
# -------------------------
...
fig3a_flour <- ggplot(df_flour_sample_plot,
...
ggsave("figures/Fig3a_flour_order_composition.png",
       plot = fig3a_flour, width = 6.85, height = 3.4, dpi = 300, bg = "white")

# -------------------------
# Figure 3b — gut microbiome: SAMPLE-LEVEL bars (Reviewer 3)
# -------------------------
...
fig3b_gut <- ggplot(df_gut_sample_plot,
                    aes(x = Sample, y = Relative, fill = Order)) +
...
ggsave("figures/Fig3b_adultgut_order_composition.png",
       plot = fig3b_gut, width = 8.5, height = 6.0, dpi = 300, bg = "white")

# -------------------------
# Figure 4 — alpha diversity dynamics
# -------------------------
...
ggsave("figures/Fig4_alpha_diversity_dynamics.png",
       plot = fig4, width = 6.85, height = 6.4, dpi = 300, bg = "white")

# -------------------------
# Figure 5 — TWO-PANEL PCoA: all generations + G1-G3 only (Reviewer 3)
# -------------------------
...
fig5a <- plot_ordination(ps_AdultGut, pcoa_bray, color = "Generation") +
...
fig5b <- plot_ordination(ps_G1G3, pcoa_G1G3, color = "Generation") +
  geom_point(aes(shape = Treatment), size = 2.6, alpha = 0.9) +
  stat_ellipse(aes(group = Generation), type = "t",
               linetype = "dashed", linewidth = 0.4, show.legend = FALSE) +
  scale_shape_manual(values = trt_shapes,   drop = FALSE, name = "Treatment") +
...
fig5_combined <- fig5a + fig5b +
  plot_layout(guides = "collect") &
  theme(legend.position = "right")

ggsave("figures/Fig5_PCoA_AdultGut.png",
       plot = fig5_combined, width = 13.0, height = 4.8, dpi = 300, bg = "white")
...
png(filename = "figures/Fig6_diet_shift_heatmap.png",
...
# -------------------------
# Figure 7 — Shannon diversity trajectories
...
ggsave("figures/Fig7_shannon_diversity_trajectories.png",
       plot = fig7, width = 6.85, height = 4.8, dpi = 300, bg = "white")
